...
const ERROR = {
    ...
    SERVER: "EInternal server error",   
}

export const POST: APIRoute = async ({ request }) => {
//    const { name, email, subject, message } = await request.body() || undefined
    ...
    try {
        ...
        return new Response(JSON.stringify({
            success: true,
            message: "Menssage sent successfully"
        }))

    } catch(e) {
        console.error(e)
        return new Response(
            JSON.stringify({message: ERROR.SERVER}), {status: 500}
        )
    }
}
